...
使用方法:
1. 設定
   - config.jsonで監視地点と通知設定
   - Windowsの場合Outlookの設定
   - 必要なPythonパッケージのインストール

2. 実行
   - 通常実行: python monitor.py
   - 1回のみ: python monitor.py --once
   - メール設定テスト: python monitor.py --test-email

3. 監視
   - ログファイル: logs/monitor.log
   - 稼働状態: logs/monitor_heartbeat.json
   - プロセス管理: logs/monitor.pid
"""
...
# ───────── JMA配色と変換 ─────────
"""
気象庁（JMA）の降水強度データの解析と変換

データ形式:
1. ステップ値（0-65）
   - 1-60: 直接的な降水量（mm/h）
   - 61-65: 特殊な強度値（80-300mm/h）
...
# 降水強度のステップ値から実際の降水量（mm/h）への変換マップ
STEP_TO_MM_IDENTITY = {a: a for a in range(1, 61)}  # 1-60は直接的なmm/h値
STEP_TO_MM_IDENTITY.update({  # 61-65は特殊な強度値
    61: 80,   # 強い雨
    62: 100,  # 激しい雨
    63: 150,  # 非常に激しい雨
    64: 200,  # 猛烈な雨
...
class NotificationManager:
    ...
    Attributes:
        cfg (Dict[str, Any]): システム設定
        db_path (str): データベースファイルパス
        mailer (Optional[OutlookMailer]): メール送信クライアント
    """
